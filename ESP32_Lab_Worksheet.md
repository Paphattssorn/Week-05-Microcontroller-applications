...
```c
- เครื่องมือหลักคือ `idf.py` ซึ่งเป็น command-line tool ของ ESP-IDF  
  ใช้สำหรับ build, flash, monitor และ clean project  
  เช่น คำสั่ง `idf.py build` ใช้คอมไพล์โปรแกรม ESP32
```

3. **New Tools**: เครื่องมือใหม่ที่ติดตั้ง (tree, htop) ใช้ทำอะไร?
```c
- `tree` : ใช้แสดงโครงสร้างไฟล์และโฟลเดอร์ในรูปแบบลำดับชั้น (hierarchical view)  
- `htop` : ใช้ดูการทำงานของระบบ เช่น CPU usage, Memory usage, Process ที่กำลังรันอยู่ใน container
```

4. **Architecture Focus**: การศึกษา ESP32 architecture แตกต่างจากการทำ arithmetic ใน Lab4 อย่างไร?
```c
- Lab4 เน้นการทดสอบการคำนวณ (arithmetic) และการ build โค้ดพื้นฐานใน ESP-IDF  
- Lab5 เน้นการวิเคราะห์เชิงสถาปัตยกรรม เช่น หน่วยความจำ (memory layout), cache performance และระบบ dual-core  
  เพื่อเข้าใจการทำงานภายในของ ESP32 มากขึ้น
```
...
```c
- SRAM: ใช้เก็บข้อมูลที่เปลี่ยนแปลงได้ระหว่างรัน เช่น ตัวแปรใน stack, heap, และ global variables  
- Flash Memory: ใช้เก็บข้อมูลถาวร เช่น โปรแกรม (code), ค่าคงที่ (constant), และ string literals
```
 
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
```c
- Stack (ตัวแปรในฟังก์ชัน): เริ่มต้นที่ 0x3FFBxxxx  
- Heap / Global (DRAM): อยู่ช่วง 0x3FFCxxxx  
- Flash (.rodata / code): อยู่ช่วง 0x400xxxxx  
- PSRAM (ถ้ามี): อยู่ช่วง 0x3F80xxxx ถึง 0x3FBFFFFF

```
 
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
```c
- Internal SRAM รวมประมาณ 520 KB  
- ใช้ไปบางส่วนสำหรับ heap, stack และ data ประมาณ 380 KB (ตามตัวอย่างในตาราง)  
- Flash memory มีขนาดใหญ่กว่ามาก (หลาย MB) ใช้เก็บโปรแกรมหลัก  
  โดยใช้ไปบางส่วนตามขนาดของ binary file ที่ build ได้
...
- เพราะ sequential access อ่านข้อมูลแบบต่อเนื่อง ทำให้ cache สามารถโหลดข้อมูลล่วงหน้า (prefetch) ได้  
- ส่วน random access จะกระโดดไปตำแหน่งต่าง ๆ ทำให้ cache miss บ่อย จึงใช้เวลามากกว่า
- 
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?

- Internal SRAM: อยู่ในชิปหลักของ ESP32 ความเร็วสูงกว่าและ latency ต่ำ  
- External Memory (PSRAM): ต่อภายนอกผ่าน bus ทำให้ช้ากว่า มี latency สูงกว่า  
  แต่ข้อดีคือมีความจุมากกว่า เหมาะสำหรับงานที่ต้องใช้หน่วยความจำเยอะ เช่น buffer หรือ image data

3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?

- stride ขนาดเล็ก (เช่น 1, 2) เข้าถึงข้อมูลใกล้กัน ทำให้ cache hit สูงและเร็วกว่า  
- stride ขนาดใหญ่ (เช่น 8, 16) ข้ามข้อมูลหลายช่อง ทำให้ cache ไม่ได้ใช้อย่างมีประสิทธิภาพและช้าลง  
  ดังนั้น performance จะลดลงเมื่อ stride ใหญ่ขึ้น
...
- Core 0 (PRO_CPU): เหมาะกับงานที่เกี่ยวข้องกับระบบ (System tasks) หรือ protocol processing  
- Core 1 (APP_CPU): เหมาะกับงานทั่วไป เช่น Application logic, computation, หรือ UI  
  โดยทั่วไป Core 0 จะควบคุมระบบหลัก ส่วน Core 1 จะช่วยแบ่งเบาภาระในการประมวลผล

2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?

- มี overhead เล็กน้อยจากการใช้ Queue และ Semaphore  
  โดย latency อาจอยู่ในระดับหลักหลายร้อยถึงหลักพันไมโครวินาที (μs)  
  ขึ้นอยู่กับจำนวนข้อความและขนาดข้อมูลที่ส่งระหว่าง core

3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?

- มีประสิทธิภาพเมื่อแต่ละ core ได้รับงานที่เหมาะสม เช่น core0 ทำงาน protocol / I/O  
  และ core1 ทำงานคำนวณหรือ application  
- ถ้างานไม่สมดุล (core ใด core หนึ่งทำงานหนักกว่า) จะเห็นได้จากค่า iteration/time ที่ต่างกันมาก  
  ควรปรับ scheduling หรือ priority ให้เหมาะสม
...
**ข้อมูลนักศึกษา:**
- ชื่อ: ปภัสสร เอี่ยมสอาด
- รหัสนักศึกษา: 67030311
- วันที่ทำการทดลอง: 10/29/68
...
1. เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:
 - สัปดาห์ที่ 4 เป็นครั้งแรกที่ใช้ Docker สำหรับ ESP-IDF setup  
  ยังต้องเรียนรู้คำสั่งและการ mount โฟลเดอร์  
- สัปดาห์ที่ 5 ใช้ environment เดิม จึงทำงานได้เร็วขึ้น  
  เข้าใจการ exec เข้า container และ build project ได้คล่องกว่าเดิม

2. สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 architecture:
   - เข้าใจการทำงานของ Dual-Core CPU (PRO_CPU, APP_CPU)  
- เห็นภาพโครงสร้างหน่วยความจำ (Flash, SRAM, Heap, Stack)  
- เข้าใจหลักการ cache และผลของ cache hit/miss ต่อประสิทธิภาพ  
- เรียนรู้การใช้ QEMU จำลอง ESP32 โดยไม่ต้องมี hardware จริง

3. ความท้าทายที่พบในการทำ architecture analysis:
   - ต้องใช้เครื่องมือหลายตัว เช่น Docker, ESP-IDF, QEMU, objdump  
- ผลการจำลองบางอย่างไม่ตรงกับ hardware จริง  
- ต้องเข้าใจ address mapping และประเภทหน่วยความจำของ ESP32 อย่างละเอียด  
- เวลา build project ขนาดใหญ่ อาจใช้เวลานานและต้องใช้ RAM เยอะ
